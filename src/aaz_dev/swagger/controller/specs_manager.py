...
class SwaggerSpecsModuleManager:
    ...
    def __init__(self, plane, module):
        ...
        assert plane == PlaneEnum.Mgmt or PlaneEnum.Fllm or PlaneEnum.is_data_plane(plane), f"Invalid plane: '{self.plane}'"
    ...
    def get_resource_in_version(self, resource_id, version, rp_name=None, plane=None):
        
        if (plane == PlaneEnum.Fllm):
            #strip out the proper resource provider from resource_id
            vals = resource_id.split('/')

            #get the one with foundationallm in it
            for val in vals:
                if 'foundationallm' in val:
                    vals = val.split('.')
                    name = vals[1]
                    #set first char to upper case
                    name = name[0].upper() + name[1:]
                    rp_name = f"FoundationaLLM.{name}"
                    break
...
class SwaggerSpecsManager:
    ...
    def get_swagger_resource(self, plane, mod_names, resource_id, version):
        ...
        ).get_resource_in_version(resource_id, version, plane=plane)
