...
class AzModuleManager:
    ...
    def _patch_module(self, mod_name):
        ...
        insert_lines = [
            f"{space}{space}from foundationallm.cli.core.aaz import load_aaz_command_table",
            f"{space}{space}try:",
...
class AzExtensionManager(AzModuleManager):
    ...
    def _find_module_folder(cls):
        ...
        if not cls._folder:
            ...
            for path, _, _ in os.walk(cli_ext_folder):
                if 'fllmext_' in path:
                    continue
                ext_paths = [path for path in glob.glob(os.path.join(path, 'fllmext_*')) if os.path.isdir(path)]
    ...
    def _generate_mod_ext_name(self, mod_name):
        return 'fllmext_' + self.pkg_name(mod_name)
    ...
    def get_mod_ext_path(self, mod_name):
        mod_path = self.get_mod_path(mod_name)
        ext_paths = [path for path in glob.glob(os.path.join(mod_path, 'fllmext_*')) if os.path.isdir(path)]
        if not ext_paths:
            return os.path.join(mod_path, self._generate_mod_ext_name(mod_name))
        else:
            if len(ext_paths) > 1:
                raise exceptions.ResourceConflict(f"Find multi folders start with 'fllmext_' in module path '{mod_path}'")
            return ext_paths[0]
    ...
    def create_or_update_mod_azext_metadata(self, mod_name):
        ...
        metadata_path = os.path.join(ext_path, 'fllmext_metadata.json')
        if os.path.exists(metadata_path):
            with open(metadata_path, 'r', encoding="utf-8") as f:
                metadata = json.load(f)
            if "fllmext.minCliCoreVersion" not in metadata or \
                    version.parse(metadata["fllmext.minCliCoreVersion"]) < Config.MIN_CLI_CORE_VERSION:
                metadata["fllmext.minCliCoreVersion"] = str(Config.MIN_CLI_CORE_VERSION)
        else:
            metadata = {
                "fllmext.isPreview": True,
                "fllmext.minCliCoreVersion": str(Config.MIN_CLI_CORE_VERSION),
            }
    ...
    def create_new_mod(self, mod_name):
        ...
        # fllmext_* folder
        ext_path = self.get_mod_ext_path(mod_name)
        ext_templates = templates['fllmext_']
        ...
        # fllmext_metadata.json
        file_path = os.path.join(ext_path, 'fllmext_metadata.json')
        if os.path.exists(file_path):
            raise exceptions.ResourceConflict(f"File already exist: '{file_path}'")
        file_path, ext_metadata = self.create_or_update_mod_azext_metadata(mod_name)
