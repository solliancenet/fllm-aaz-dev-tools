...
class CliExtensionTemplateRenderTest(CommandTestCase):
    ...
    def test_render_azext_base(self):
        mod_name = "az-firewall"
        azext_name = f"fllmext_{mod_name.replace('-', '_')}"

        tmpl = get_templates()['extension']['fllmext_']['__init__.py']
        data = tmpl.render(
            mod_name=mod_name
        )
        ...
        tmpl = get_templates()['extension']['fllmext_']['_help.py']
        data = tmpl.render()
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", mod_name,
                                   azext_name, "_help.py")
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        ...
        tmpl = get_templates()['extension']['fllmext_']['_params.py']
        data = tmpl.render()
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", mod_name,
